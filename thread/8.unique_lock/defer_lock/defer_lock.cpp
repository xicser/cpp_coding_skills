...
defer_lock的意思就是并没有给mutext加锁: 初始化了一个没有加锁的mutex, 相当于一种绑定, 将unique_lock和mutex绑定在一起
...
转移:
std::unique_lock<std ::mutex> sbguard2(move(sbguard1)); //调用了移动构造函数, 现在sbguard1指向空，sbguard2指向了mutex
